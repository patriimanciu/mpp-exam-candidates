...
async function generateBulkNews() {
    console.log(`Starting bulk news generation: ${NEWS_ITEMS_PER_VOTER} items per voter.`);
    ...
    try {
        ...
        if (voters.length === 0 || candidates.length === 0) {
            throw new Error('Database must contain at least one voter and one candidate.');
        }

        let totalNewsGenerated = 0;
        for (const voter of voters) {
            const existingNewsResult = await client.query('SELECT news_text FROM FakeNews WHERE voter_cnp = $1', [voter.cnp]);
            const existingNews = new Set(existingNewsResult.rows.map(r => r.news_text));
            
            let newsGeneratedForVoter = 0;
            let attempts = 0;
            while (newsGeneratedForVoter < NEWS_ITEMS_PER_VOTER && attempts < 100) {
                ...
                if (!existingNews.has(newsText)) {
                    await client.query(
                        'INSERT INTO FakeNews (voter_cnp, news_text, is_positive, candidate) VALUES ($1, $2, $3, $4)',
                        [voter.cnp, newsText, isPositive, randomCandidate.name]
                    );
                    existingNews.add(newsText);
                    newsGeneratedForVoter++;
                    totalNewsGenerated++;
                }
                attempts++;
            }
            console.log(`Generated ${newsGeneratedForVoter} news items for voter ${voter.cnp}.`);
        }

        await client.query('COMMIT');
        
        console.log('\n--- Bulk News Generation Complete ---');
        console.log(`Successfully generated a total of ${totalNewsGenerated} new items.`);
        console.log('-------------------------------------\n');

    } catch (err) {
        await client.query('ROLLBACK');
        console.error('An error occurred during bulk news generation:', err);
    } finally {
        client.release();
        pool.end();
    }
